...
from inferix.core.types.inference import (
    InferenceParams,
    ModelMetaArgs,
...
from inferix.core.checkpoint import load_inference_checkpoint
from inferix.distributed import parallel_state as mpu
from inferix.distributed.parallelism import cp_post_process, cp_pre_process, pp_scheduler
...
def get_dit(config: MagiConfig):
    ...
    if not env_is_true("SKIP_LOAD_MODEL"):
        model = load_inference_checkpoint(model)
        mem_allocated_gb = torch.cuda.memory_allocated() / 1024**3
        mem_reserved_gb = torch.cuda.memory_reserved() / 1024**3
        print_rank_0(

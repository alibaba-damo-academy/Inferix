...
class SelfForcingKVCacheManager:
    """
    Standalone KV Cache management module for Self-Forcing model inference.
    
    This module handles the allocation, storage, and retrieval of key-value pairs
    during Self-Forcing transformer inference to enable efficient autoregressive generation.
    Compatible with both CausalInferencePipeline and CausalDiffusionInferencePipeline.
    """
    ...
    def set_kv_cache(self, kv_cache_manager: KVCacheManager, kv_cache_request: KVCacheRequest, 
        ...
        """
        # Split k and v if they're concatenated
    ...
    def get_crossattn_cache(self, kv_cache_manager: KVCacheManager, kv_cache_request: KVCacheRequest) -> torch.Tensor:
        ...
        """
        return kv_cache_manager.get(kv_cache_request, f"crossattn_layer_{self.layer_number}")
...
class SelfForcingKVCacheManagerFactory:
    """
    Factory class for creating SelfForcingKVCacheManager instances.
    """
    
    @staticmethod
    def create_manager(layer_number: int, num_query_groups_per_partition: int = 12, 
                      hidden_size_per_attention_head: int = 128, 
                      enable_kv_offload: bool = False) -> SelfForcingKVCacheManager:
        """
        Create a SelfForcingKVCacheManager instance.
        ...
        Returns:
            SelfForcingKVCacheManager instance
        """
        return SelfForcingKVCacheManager(
    ...
    @staticmethod
    def create_managers(num_layers: int, num_query_groups_per_partition: int = 12, 
                       hidden_size_per_attention_head: int = 128, 
                       enable_kv_offload: bool = False) -> list[SelfForcingKVCacheManager]:
        """
        Create multiple SelfForcingKVCacheManager instances for all layers.
        ...
        Returns:
            List of SelfForcingKVCacheManager instances
        """
        return [
            SelfForcingKVCacheManagerFactory.create_manager(
                layer_number=i,
                num_query_groups_per_partition=num_query_groups_per_partition,
                hidden_size_per_attention_head=hidden_size_per_attention_head,
                enable_kv_offload=enable_kv_offload

"""Inference-specific checkpoint loading utilities.

This module provides optimized checkpoint loading for production inference,
including support for sharded parallel loading, compressed weights, and
pipeline parallelism weight distribution.
"""
...
import inferix.distributed.parallel_state as mpu
from inferix.core.config import EngineConfig, ModelConfig, RuntimeConfig
from inferix.distributed.dist_utils import print_per_rank, print_rank_0


def _load_shard(shard_path, param_names, num_threads=None):
    """Load a single shard with optional decompression."""
...
def load_sharded_safetensors_parallel_with_progress(checkpoint_dir):
    """Load sharded safetensors files in parallel with progress tracking."""
...
def unwrap_model(model):
    """Unwrap model from potential distributed wrappers."""
    return_list = True
...
def _split_state_dict_for_pp(weight_dict: OrderedDict, model_config: ModelConfig):
    """Split state dict for pipeline parallelism."""
...
def load_inference_state_dict(runtime_config: RuntimeConfig, engine_config: EngineConfig):
    """Load state dict for inference with support for quantization and distillation."""
...
def load_inference_checkpoint(model):
    """Load checkpoint for inference with optimization for production use."""
    state_dict = load_inference_state_dict(model.runtime_config, model.engine_config)

...
class AttrDict(dict):
    ...
    def __setattr__(self, name, value):
        if name in self.__dict__: # Allow setting internal attributes
    ...
    def __setitem__(self, key, value):
        # When setting an item, ensure nested dicts are converted
...
def load_py_config_without_mmengine(config_path):
    """
    Loads a Python-based configuration file and returns an AttrDict object.
    This function does NOT handle `_base_` or `_import_` directives
    like MMEngine. It simply executes the Python file and extracts
    top-level variables.
    ...
    Returns:
        AttrDict: An AttrDict object representing the configuration.
    
    Raises:
        FileNotFoundError: If the config file does not exist.
        SyntaxError: If the config file has Python syntax errors.
        Exception: For other errors during execution.
    """
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"Config file not found: {config_path}")

    # Prepare a namespace for executing the config file
    config_vars = {}
    
    # Store current working directory and sys.path for restoration
    original_cwd = os.getcwd()
    original_sys_path = sys.path[:]

    try:
        # Change CWD to config file's directory to resolve relative imports if any
        config_dir = os.path.dirname(os.path.abspath(config_path))
        if config_dir: # if not current directory
            os.chdir(config_dir)
            sys.path.insert(0, config_dir) # Add to sys.path for imports

        # Read the content of the config file
        with open(config_path, 'r', encoding='utf-8') as f:
            config_code = f.read()

        # Execute the config code in the `config_vars` namespace
        # The `exec` function puts top-level variables defined in config_code
        # into the `config_vars` dictionary.
        exec(compile(config_code, config_path, 'exec'), {}, config_vars)

        # Filter out built-in variables (like __file__, __name__, etc.)
        # and any MMEngine-specific internal variables if they happen to be there.
        filtered_config = {
            k: v for k, v in config_vars.items()
            if not k.startswith('__') and not isinstance(v, (types.ModuleType, types.FunctionType))
        }

        return AttrDict(filtered_config)
...
def read_config(config_path):
    ...
    cfg = load_py_config_without_mmengine(config_path)
    print("cfg : ")
    print(cfg)
    print("")
    return cfg
